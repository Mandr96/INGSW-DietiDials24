...
public class DemoApplication {
	...
	CommandLineRunner commandLineRunner(UtentiRepository userRep, AsteRepository asteRep) {
		return args -> {
			...
			userRep.save(user);
			Asta classica = new AstaClassica(1L, Timestamp.from(Instant.now()), "cosa1", "una cosa", null, false, user, new ArrayList<Offerta>(), 0f);
			Asta silenziosa = new AstaSilenziosa(2L, Timestamp.from(Instant.now()), "cosa2", "una cosa", null, false, user, new ArrayList<Offerta>());
